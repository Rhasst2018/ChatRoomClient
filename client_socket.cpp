...
void client_socket::read_msg()
{
    ...
    quint16 is_changedOK;
    QString displsyname;

    QString msg_from;   ///信息发送这
    QString myname; ///接收这本人阿
    quint16 port;/////文件传输时候的服务器的端口号
    QString filename;
    QString msg;
...
////////以上定义的为接收到的信息的具体的格式///////////////////////////////////
    ...
    case 1:
        ...
        else if(is_canlogin==quint16(2))
            QMessageBox::warning(0, tr("密码错误"),tr("请重新输入密码！"), QMessageBox::Ok);
            //QMessageBox::information(this,"pssword_error",tr("密码错误！"),QMessageBox::Ok,QMessageBox::Close);
        else
            QMessageBox::warning(0, tr("警告"),tr("该用户已登录！"), QMessageBox::Ok);
        break;
    case 2:
        in>>is_canregister;
    ...
    case 7:
         ...
         in>>msg_from>>myname>>port>>addr>>filename>>filesize;
         qDebug()<<"msg from "<<msg_from<<"myname is "<<myname<<"port is "<<port<<"filename is "<<filename;///客户端新建立链接即将
         emit should_connect(msg_from,myname,port,addr,filename,filesize);
         break;
    case 9:
        ...
        break;
    case 10:
        qDebug("i got the 10 in the client ");
        in>>msg_from>>myname;///信息来源和信息接受这就是本人自己
        emit close_filetrans(msg_from,myname);
        break;
    case 11:
        qDebug("i got the 11 in the client");
        in>>is_changedOK;
        emit changeOK(is_changedOK);
        break;
    case 12:
        qDebug("i got the 12 in the client");
        in>>msg_from>>myname>>re_addr>>re_port;
        emit assure_filetrans(msg_from,myname,re_addr,re_port);
    default:
        break;
    }
}
...
void client_socket::get_sendip(QString myname, QString sendname, quint16 portt,QHostAddress addr,QString filename,qint64 filesize)
{
    QByteArray buffer;
    QDataStream out(&buffer,QIODevice::WriteOnly);
    out<<quint16(7);//获取一个客户端的ip和端口
    out<<myname<<sendname<<portt<<addr<<filename<<filesize;
    this->write(buffer);
    qDebug()<<"now i am doing the things that i want to get the kefuduan de ip and port!!!!!!!!!!!!!!!!!!!----159"<<myname<<"---"<<sendname;
    return;
}

void client_socket::assure_receive(QString msg_from, QString name,QHostAddress addr, quint16 re_port)
{
    QByteArray buffer;
    QDataStream out(&buffer,QIODevice::WriteOnly);
    out<<quint16(12);////表示是接收文件的信息
    out<<name<<msg_from<<addr<<re_port;
    this->write(buffer);
    return;
}
...
void client_socket::receive_success(QString msg_from, QString myname, QHostAddress addr, quint16 port)
{
    qDebug()<<"fail has been receive success !!!"<<"--and msg_from is "<<msg_from<<"myname is "<<myname;
    QByteArray buffer;
    QDataStream out(&buffer,QIODevice::WriteOnly);
    out<<quint16(10);////表示接收文件成功的信号
    out<<myname<<msg_from<<addr<<port;
    this->write(buffer);
    return;
}
